...
import { asset } from "../../serializer/src/operations";
...
class ChainStore {
    ...
    clearCache() {
        ...
        */
        ...
        this.liquidity_pools_by_asset = new Map();
    ...
    getLiquidityPoolByID(pool_id) {
        if (!ChainValidation.is_object_id(pool_id))
            throw Error("argument is not an object id: " + id);

        let pool_obj = this.objects_by_id.get(pool_id);
        if (pool_obj === undefined) {
            return this.getObject(pool_id);
        }
        return pool_obj;
    }
